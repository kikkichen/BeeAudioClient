...
@HiltViewModel
class JoinPremiumFamilyVM @Inject constructor(
    ...
    fun loadPremiumGroupInfo() {
        viewModelScope.launch {
            try {
                _premiumGroup.value = localPremiumApi.getPremiumCardInfo(cardId = _code.value).data
            } catch (e : Throwable) {
    ...
    fun postJoinPremiumGroupApply(context: Context, currentUserId: Long) {
        viewModelScope.launch {
            try {
                val result = localPremiumApi.postJoinInPremiumGroup(targetId = currentUserId, cardId = _code.value)
                if (result.data) {
    ...
    /* 更新输入框内容 */
    fun changeCodeString(newCodeString: String) {
        if (newCodeString.length<=25) {
            _code.value = newCodeString.uppercase()
        }
    }
}